...
interface DebtFreeChartProps {
  ...
  /** Controlled payment value from slider (drives future3 recalc) */
...
/** Normalise all three series to the same month-index length */
...
export function DebtFreeChart({
  ...
  const data = useMemo(
    () => buildChartData(future1, future2, future3),
  ...
  const totalBalance = future1.balanceArray[0]
    ? future1.balanceArray[0] + future1.monthlyPayment
    : 0;
  ...
  return (
    <div className="flex flex-col gap-4">
          ...
          <AreaChart data={data} margin={{ top: 4, right: 4, left: -24, bottom: 0 }}>
            <defs>
              ...
              <linearGradient id="gF3" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%"  stopColor="#00C9A7" stopOpacity={0.30} />
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1.5">
        ...
        {future2 && (
          <LegendItem color="#F59E0B" label="Current pace" value={fmtMonths(future2.monthsToZero)} />
        )}
        {future3 && (
          <LegendItem color="#00C9A7" label="With Buffer" value={fmtMonths(future3.monthsToZero)} bold />
        )}
      </div>
      ...
      {onPaymentChange && (
        <div className="bg-[#1A1F2E] rounded-2xl p-4 flex flex-col gap-3">
          <div className="flex items-center justify-between">
            ...
            <p className="text-[#00C9A7] font-bold font-mono text-lg">
          ...
          <input
            ...
            aria-label="Adjust monthly payment"
            aria-valuemin={sliderMin}
            aria-valuemax={sliderMax}
            aria-valuenow={currentPayment}
            aria-valuetext={`$${currentPayment} per month`}
            className="slider w-full h-1.5 rounded-full appearance-none cursor-pointer
              bg-[#2A3040] [&::-webkit-slider-thumb]:appearance-none
              [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:h-5
              [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-[#00C9A7]
              [&::-webkit-slider-thumb]:shadow-[0_0_0_3px_rgba(0,201,167,0.25)]
              [&::-moz-range-thumb]:w-5 [&::-moz-range-thumb]:h-5
              [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:bg-[#00C9A7]
              [&::-moz-range-thumb]:border-0 focus-visible:outline-none
              focus-visible:ring-2 focus-visible:ring-[#00C9A7]"
          />

          <div className="flex items-center justify-between text-xs text-[#4A5568]">
            <span className="font-mono">${sliderMin}/mo min</span>
            {future3 && (
              <span className="text-[#00C9A7] font-mono">
...
function LegendItem({
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className={['text-xs', bold ? 'text-white font-semibold' : 'text-[#4A5568]'].join(' ')}>
        {label}:
      </span>
...
function SavingsCallout({
  interestSaved, monthsSaved, totalBalance,
}: {
  interestSaved: number; monthsSaved: number; totalBalance: number;
}) {
    ...
    <div className="bg-[#00C9A7]/10 border border-[#00C9A7]/20 rounded-2xl px-4 py-3 flex items-center gap-3">
      ...
      <div>
        <p className="text-white text-sm font-semibold">
          Save <span className="text-[#00C9A7] font-mono">${Math.round(interestSaved).toLocaleString('en-CA')}</span> in interest
        </p>
