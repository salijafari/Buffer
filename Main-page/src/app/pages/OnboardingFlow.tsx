...
export default function OnboardingFlow() {
  ...
  async function handleFinish() {
    setFinishing(true);
    // Mark onboarding complete on the Clerk user record so the entry screen
    // routes returning users directly to /dashboard on next sign-in.
    if (user) {
...
function StepKyc({ kyc, setKyc, onNext }: {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* ID file upload */}
      <div className="flex flex-col gap-1.5">
        <label className="text-[#8B9CB6] text-sm font-medium">
          Upload document <span className="text-[#4A5568] font-normal">(JPG, PNG or PDF, max 10 MB)</span>
        </label>
        <label className={[
...
function StepPad({ accepted, onAccept, onFinish, finishing }: {
  accepted: boolean;
  onAccept: () => void;
  onFinish: () => void;
  finishing: boolean;
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <PrimaryButton onClick={onFinish} disabled={!accepted || finishing}>
        {finishing ? (
          <span className="flex items-center justify-center gap-2"><SpinnerIcon />Setting up your account…</span>
